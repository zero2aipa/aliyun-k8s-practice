...
# ============================================================
step "安装 containerd、kubelet、kubeadm、kubectl"

PKG_CACHE_DIR="/opt/k8s-pkg-cache"
APT_FLAGS=(-y -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold)
apt-get update -y >/dev/null 2>&1 || true

for pkg in containerd kubelet kubeadm kubectl; do
  if [[ -f "${PKG_CACHE_DIR}/${pkg}_${K8S_VERSION}_amd64.deb" ]]; then
    dpkg -i "${PKG_CACHE_DIR}/${pkg}_${K8S_VERSION}_amd64.deb" >/dev/null 2>&1 || apt-get install -f -y >/dev/null 2>&1
  else
    apt-get install "${APT_FLAGS[@]}" "$pkg" >/dev/null 2>&1 || warn "安装 $pkg 失败"
  fi
done
ok "Kubernetes 组件安装完成"
